...
FPS = 8
MMAP_SCALE = 5
MMAP_BACKGROUND_COLOUR = (*MAZE_COLOURS[MazeCellTypes.WALL], 200)
MMAP_PATH_COLOUR = (*MAZE_COLOURS[MazeCellTypes.PATH], 200)
...
minimap_width = MMAP_SCALE * maze_width + 2
minimap_height = MMAP_SCALE * maze_height + 2
# `pygame.SRCALPHA` makes it so that transparency is respected when blitting.
minimap = pygame.Surface((minimap_width, minimap_height), flags=pygame.SRCALPHA)
minimap.fill(MMAP_BACKGROUND_COLOUR)
# Draw a thin frame around the minimap.
pygame.draw.rect(minimap, (0, 0, 0), (0, 0, minimap_width, minimap_height), 1)
# Put the minimap on the screen.
screen.blit(minimap, (MMAP_SCALE, MMAP_SCALE))
pygame.display.flip()
...
while playing:
    ...
    if repaint:
        ...
        MMS = MMAP_SCALE
        pygame.draw.rect(
            minimap, MMAP_PATH_COLOUR, (1 + old_x * MMS, 1 + old_y * MMS, MMS, MMS)
        )
        x, y = player_pos
        pygame.draw.rect(minimap, PLAYER_COLOUR, (1 + x * MMS, 1 + y * MMS, MMS, MMS))
        screen.blit(minimap, (MMS, MMS))
        pygame.display.flip()
